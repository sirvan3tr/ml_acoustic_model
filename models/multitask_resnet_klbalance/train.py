...
from models.multitask_resnet_klbalance.features import MelSpectrogramTransform
from models.multitask_resnet_klbalance.losses import MultiTaskKLBalancedLoss
from models.multitask_resnet_klbalance.model import MultiTaskResNet


@dataclass
class TrainConfig:
    data_root: str = "mic_array_data"
    output_dir: str = "models/multitask_resnet_klbalance/runs/default"
    seed: int = 42
...
def train_one_epoch(
    model: nn.Module,
    criterion: MultiTaskKLBalancedLoss,
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(
        description="Train multitask UAV model with fixed weights + side KL balancing."
    )
    parser.add_argument("--data-root", default="mic_array_data")
    parser.add_argument("--output-dir", default="models/multitask_resnet_klbalance/runs/default")
    ...
    parser.add_argument("--model-in-channels", type=int, default=16)
    parser.add_argument("--w-distance", type=float, default=1.0)
    parser.add_argument("--w-height", type=float, default=1.0)
    parser.add_argument("--w-azimuth", type=float, default=1.0)
    parser.add_argument("--w-side", type=float, default=2.0)
    ...
    parser.set_defaults(append_relative_channels=True)
...
def main() -> None:
    cfg = parse_args()
    set_seed(cfg.seed)
    expected_in_channels = 16 if cfg.append_relative_channels else 8
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        print(
            ...
            f"val_side_ce={val_stats['side_ce']:.3f} | "

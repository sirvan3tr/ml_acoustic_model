...
class KendallMultiTaskLoss(nn.Module):
    ...
    def __init__(self, side_weight_floor: float = 1.0) -> None:
        super().__init__()
        self.log_vars = nn.Parameter(torch.zeros(4))
        self.side_weight_floor = side_weight_floor
    ...
    def forward(
        ...
        side_weight = torch.clamp(task_weights[3], min=self.side_weight_floor)
        task_weights = torch.stack(
            [task_weights[0], task_weights[1], task_weights[2], side_weight]
        )
        weighted = task_weights * raw_losses + self.log_vars
        total = weighted.sum()

...
class MelSpectrogramTransform:
    ...
    and explicitly computes Inter-channel Phase Differences (IPD).
    """
    ...
    def __init__(
        ...
        self.append_relative_channels = append_relative_channels
        self.append_phase_ipd = append_phase_ipd
    ...
    def __call__(self, audio: torch.Tensor, sample_rate: int) -> torch.Tensor:
        ...
        # return_complex=True allows extracting both power and phase
        spec = torch.stft(
            x,
            n_fft=self.n_fft,
        ...
        # Global normalization preserves inter-channel level relationships
        mean = mel.mean()
        ...
        # 2. Extract Relative Channel Level Differences
        if self.append_relative_channels:
            mel_rel = mel - mel.mean(dim=0, keepdim=True)
            features.append(mel_rel)

        # 3. Extract Inter-channel Phase Differences (IPD)
        if self.append_phase_ipd:
            phase = spec.angle()  # [C, F, Frames] in radians (-pi, pi)
            
            # Map phase to mel scale using the filterbank to match the resolution
            # To do this safely with angles, we apply the filterbank on sin/cos, then arctan2
            ...
            # Compute IPD relative to the array mean phase
            # Because phase is circular, we compute relative circular difference
            mean_phase = torch.atan2(
                torch.sin(mel_phase).mean(dim=0, keepdim=True),
                torch.cos(mel_phase).mean(dim=0, keepdim=True)
            )
            
            # Subtraction in circular domain: angle(e^{i*p1} * e^{-i*p2})
            ipd = torch.remainder(mel_phase - mean_phase + math.pi, 2 * math.pi) - math.pi
            
            # Normalize IPD linearly to ~[-1, 1] for the network
            ipd_norm = ipd / math.pi
            features.append(ipd_norm)

        # Final concatenation along the channel dimension

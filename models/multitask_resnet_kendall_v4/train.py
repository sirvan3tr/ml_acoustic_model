...
from models.multitask_resnet_kendall_v4.features import MelSpectrogramTransform
from models.multitask_resnet_kendall_v4.losses import KendallMultiTaskLoss
...
class TrainConfig:
    ...
    output_dir: str = "models/multitask_resnet_kendall_v4/runs/default"
    seed: int = 42
    ...
    mel_height: int = 256
    mel_width: int = 256
    append_relative_channels: bool = True
    append_phase_ipd: bool = True
    model_in_channels: int = 24

    max_train_steps: int | None = None
    max_val_steps: int | None = None
    side_weight_floor: float = 1.0
    device: str = "cuda:1" if torch.cuda.device_count() > 1 else "cuda:0"
    ...
    for step, batch in enumerate(loader):
        ...
        metrics = {
            ...
            "azimuth_circular_mae_deg": float(
                circular_mae_deg(pred_azimuth_deg, targets["azimuth_deg"])
                .detach()
                .cpu()
            ),
            "side_accuracy": float(
                (pred_side == targets["side_class"])
                .float()
                .mean()
                .detach()
                .cpu()
...
def build_dataloaders(cfg: TrainConfig) -> tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    transform = MelSpectrogramTransform(
        ...
        append_relative_channels=cfg.append_relative_channels,
        append_phase_ipd=cfg.append_phase_ipd,
    )
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="Train V4 multitask UAV model with Phase capabilities.")
    parser.add_argument("--data-root", default="mic_array_data")
    parser.add_argument("--output-dir", default="models/multitask_resnet_kendall_v4/runs/default")
    ...
    parser.add_argument("--model-in-channels", type=int, default=24)
    parser.add_argument("--side-weight-floor", type=float, default=1.0)
    parser.add_argument(
        "--no-relative-channels",
        action="store_false",
        dest="append_relative_channels",
        help="Disable appended relative-channel mel features.",
    )
    parser.add_argument(
        "--no-phase-ipd",
        action="store_false",
        dest="append_phase_ipd",
        help="Disable appended explicitly IPD features.",
    )
    parser.set_defaults(append_relative_channels=True, append_phase_ipd=True)

    parser.add_argument("--max-train-steps", type=int, default=None)
    parser.add_argument("--max-val-steps", type=int, default=None)
    parser.add_argument("--device", default="cuda:1" if torch.cuda.device_count() > 1 else "cuda:0")
    ...
    return TrainConfig(
        ...
        append_relative_channels=args.append_relative_channels,
        append_phase_ipd=args.append_phase_ipd,
        model_in_channels=args.model_in_channels,
        max_train_steps=args.max_train_steps,
        max_val_steps=args.max_val_steps,
        side_weight_floor=args.side_weight_floor,
        device=args.device,
    )
...
def main() -> None:
    ...
    if cfg.append_relative_channels:
        expected_in_channels += 8
    if cfg.append_phase_ipd:
        expected_in_channels += 8
        
    if cfg.model_in_channels != expected_in_channels:
        raise ValueError(
            "model_in_channels does not match transform output. "
            f"Expected {expected_in_channels}, got {cfg.model_in_channels}. "
            f"(Base: 8, Relative:{'+8' if cfg.append_relative_channels else '+0'}, Phase IPD:{'+8' if cfg.append_phase_ipd else '+0'})"
        )
    ...
    model = MultiTaskResNet(in_channels=cfg.model_in_channels).to(device)
    criterion = KendallMultiTaskLoss(side_weight_floor=cfg.side_weight_floor).to(device)

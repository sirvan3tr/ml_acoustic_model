...
class KendallMultiTaskLoss(nn.Module):
    ...
    @staticmethod
    def _azimuth_cosine_loss(pred_deg: torch.Tensor, target_deg: torch.Tensor) -> torch.Tensor:
        pred_rad = torch.deg2rad(pred_deg)
        target_rad = torch.deg2rad(target_deg)
        return (1.0 - torch.cos(pred_rad - target_rad)).mean()
    ...
    def forward(
        ...
        loss_azimuth = self._azimuth_cosine_loss(outputs["azimuth_deg"], targets["azimuth_deg"])

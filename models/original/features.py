...
class OriginalMelTransform:
    """
    Original paper implementation: Converts waveform [channels, time] into log-mel [channels, H, W].
    Parameters fixed to standard from paper: 44.1 kHz, 1024 n_fft, 256 hop_length, 256 mels.
    """
    ...
    def __call__(self, audio: torch.Tensor, sample_rate: int) -> torch.Tensor:
        ...
        # Baseline typically applies simple standardization or mean/std normalization per batch,
        # but maintaining the global normalization for parity here.
        mean = mel.mean()
        std = mel.std().clamp_min(1e-5)
        mel = (mel - mean) / std

        # The original stacked 8 channels.
        return mel

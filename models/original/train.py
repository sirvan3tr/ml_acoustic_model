...
from models.original.features import OriginalMelTransform
from models.original.losses import OriginalMSELoss
...
class TrainConfig:
    ...
    output_dir: str = "models/original/runs/default"
    seed: int = 42
...
def side_class_to_xy(side_class: torch.Tensor) -> torch.Tensor:
    # 0: Front (0 deg), 1: Right (90 deg), 2: Back (180 deg), 3: Left (270 deg)
...
def move_to_device(batch: dict[str, object], device: torch.device) -> tuple[torch.Tensor, torch.Tensor, dict[str, torch.Tensor]]:
    ...
    # Convert Side Class (0,1,2,3) to Sin/Cos to match Azimuth's encoding into 6 values
    side_xy = side_class_to_xy(side_class)

    # 6 continuous values required by the MSE loss:
    # [Distance, Height, Sin(Az), Cos(Az), Sin(Side), Cos(Side)]
    targets_6d = torch.stack([
        regression[:, 0],     # Distance
        regression[:, 1],     # Height
        azimuth_xy[:, 0],     # Sin(Azimuth)
        azimuth_xy[:, 1],     # Cos(Azimuth)
        side_xy[:, 0],        # Sin(Side)
        side_xy[:, 1],        # Cos(Side)
    ], dim=1)
...
def evaluate(
    ...
    for step, batch in enumerate(loader):
        ...
        # Decode the 6D output back to metrics for evaluation
        ...
        # side_xy back to side_class (0,1,2,3)
        # Find closest angle among (0, 90, 180, 270)
        pred_side_deg = azimuth_xy_to_deg(pred_side_xy)
        # 0=0, 1=90, 2=180, 3=270. Quantize by rounding divided by 90
...
def build_dataloaders(cfg: TrainConfig) -> tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    transform = OriginalMelTransform(
        n_fft=cfg.n_fft,
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="Train original paper baseline model.")
    parser.add_argument("--data-root", default="mic_array_data")
    parser.add_argument("--output-dir", default="models/original/runs/default")
...
def main() -> None:
    ...
    model = OriginalPaperResNet(in_channels=8).to(device)
    criterion = OriginalMSELoss().to(device)

    # Beta 1 = 0.5 per the paper
    optimizer = Adam(
    ...
    # Paper specified ReduceLROnPlateau
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        score = float(val_stats["total_mse"])
        ...
        print(
            f"Epoch {epoch:03d} | "
            f"train_mse={train_stats['total_mse']:.4f} | "
            f"val_mse={val_stats['total_mse']:.4f} | "
